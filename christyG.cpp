...
Image img[5] = {"images/castle2.png","images/castlebricks3.jpg", "images/grasstest_1.png","images/zomb2.png", "images/logo.png"};
GLuint castleTex;
GLuint barTex;
GLuint backTex;
GLuint zombieTex;
...
void image_opengl(void)
{
    ...
    unsigned char *castleData = buildAlphaData(castleImage);
    glBindTexture(GL_TEXTURE_2D, castleTex);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0,GL_RGBA, GL_UNSIGNED_BYTE, castleData);
    //---------------------
    //bar
    w = barImage->width;
    h = barImage->height;
    unsigned char *barData = buildAlphaData(barImage);
    glBindTexture(GL_TEXTURE_2D, barTex);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0,GL_RGBA, GL_UNSIGNED_BYTE, barData);
   ...
   // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0,GL_RGB, GL_UNSIGNED_BYTE, logoData);
...
void logoTexture(int x, int y){
    //---------logo
    glClearColor(0.82, 0.82, 0.82,0.5);
